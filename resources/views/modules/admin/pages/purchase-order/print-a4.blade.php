...
@section('content')
  @if (!$is_pdf_export)
    <div class="no-print text-center" style="margin-top: 10px; margin-bottom:20px;">
      <a class="btn my-xs" href="{{ route('admin.purchase-order.add') }}">+ Order Baru</a>
      <a class="btn my-xs" href="{{ route('admin.purchase-order.index') }}">&laquo; Daftar Order</a>
      <a class="btn my-xs" href="{{ route('admin.purchase-order.detail', $item->id) }}">&laquo; Rincian</a>
      <a class="btn my-xs" href="{{ route('admin.purchase-order.print', $item->id) }}?output=pdf&size=a4">Simpan PDF</a>
  ...
  <div class="page">
    ...
    <x-admin.company-header :logo-path="$logo_path">
      <h4 style="margin: 0 0 5px 0; text-align: Left;">PEMBELIAN {{ $item->code }}</h4>
      <table>
        <tr>
          <td>Waktu</td>
          <td>:</td>
