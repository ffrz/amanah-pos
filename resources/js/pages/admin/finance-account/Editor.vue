        ...
        <q-form class="row" @submit.prevent="submit" @validation-error="scrollToFirstErrorField">
          <q-card square flat bordered class="col">
            <q-card-section class="q-pt-none">
              ...
              <q-input autofocus v-model.trim="form.name" label="Nama Akun" lazy-rules :error="!!form.errors.name"
                :disable="form.processing" :error-message="form.errors.name" :rules="[
                  (val) => (val && val.length > 0) || 'Nama harus diisi.',
                ]" />
              <q-select v-model="form.type" label="Jenis Akun" :options="types" map-options emit-value
                :error="!!form.errors.type" :disable="form.processing">
              </q-select>
              <template v-if="form.type == 'bank'">
                <q-input v-model.trim="form.bank" label="Nama Bank" lazy-rules :error="!!form.errors.bank"
                  :disable="form.processing" :error-message="form.errors.bank" />
                <q-input v-model.trim="form.number" label="Nomor Rekening" lazy-rules :error="!!form.errors.number"
                  :disable="form.processing" :error-message="form.errors.number" />
                <q-input v-model.trim="form.holder" label="Pemilik Rekening" lazy-rules :error="!!form.errors.holder"
                  :disable="form.processing" :error-message="form.errors.holder" />
              </template>
              <LocaleNumberInput v-model:modelValue="form.balance" label="Saldo" lazyRules :disable="form.processing" allow-negative="true"
                :error="!!form.errors.balance" :errorMessage="form.errors.balance" :rules="[]" />
