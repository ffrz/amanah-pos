...
<template>
  ...
  <authenticated-layout>
    ...
    <div class="q-pa-sm">
      ...
      <div v-if="false" class="q-pt-md">
        <div class="text-subtitle1 text-bold text-grey-8">
          Statistik
          {{month_options.find((a) => a.value == selected_month).label}}
        </div>
      </div>
      <div v-if="false" class="q-pt-sm">
        <div class="row q-col-gutter-sm">
          <div class="col-md-6 col-12">
            <top-card class="full-width full-height" :items="$page.props.data.top_customers" title="Top 5 Santri"
              route_url="admin.customer.detail" />
          </div>
          <div class="col-md-6 col-12">
            <top-card class="full-width full-height" :items="$page.props.data.top_technicians" title="Top 5 Teknisi"
              route_url="admin.customer.detail" />
          </div>
        </div>
      </div>
