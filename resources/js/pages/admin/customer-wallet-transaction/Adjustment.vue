...
watch(() => form.customer_id, async (newCustomerId) => {
  ...
  try {
    const res = await axios.get(route('admin.customer.balance', { id: newCustomerId }));
...
const submit = () => handleSubmit({ form, url: route('admin.customer-wallet-transaction.adjustment') });
        ...
        <q-form class="row" @submit.prevent="submit" @validation-error="scrollToFirstErrorField">
          <q-card square flat bordered class="col">
            <q-card-section class="q-pt-none">
              ...
              <LocaleNumberInput v-model:modelValue="form.old_balance" label="Saldo Tercatat" readonly lazyRules
                :disable="form.processing" :error="!!form.errors.balance" :errorMessage="form.errors.balance" />
              <LocaleNumberInput v-model:modelValue="form.new_balance" label="Saldo Seharusnya" lazyRules
                :disable="form.processing" :error="!!form.errors.new_balance" :errorMessage="form.errors.new_balance" />
              <LocaleNumberInput v-model:modelValue="amount" label="Selisih" readonly lazyRules
