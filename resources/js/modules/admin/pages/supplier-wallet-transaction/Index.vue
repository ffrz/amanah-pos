...
const title = "Transaksi Deposit Suppllier";
const $q = useQuasar();
...
const deleteItem = (row) =>
  handleDelete({
    ...
    url: route("admin.supplier-wallet-transaction.delete", row.id),
...
const fetchItems = (props = null) => {
  ...
  handleFetchItems({
    ...
    props,
    rows,
    url: route("admin.supplier-wallet-transaction.data"),
    loading,
    tableRef,
  });
};
...
<template>
  ...
  <authenticated-layout>
    ...
    <template #right-button>
      ...
      />
      <q-btn
        v-if="$can('admin.supplier-wallet-transaction.add')"
        icon="add"
        dense
        rounded
        class="q-ml-sm"
        color="primary"
        @click="router.get(route('admin.supplier-wallet-transaction.add'))"
      />
    </template>
      ...
      >
        ...
        <template v-slot:body="props">
          <q-tr
            ...
            @click.stop="
              router.get(
                route('admin.supplier-wallet-transaction.detail', {
                  id: props.row.id,
                })
              )
            "
          >
            <q-td key="code" :props="props" class="wrap-column">
              ...
              <template v-if="!$q.screen.gt.sm">
                ...
                <LongTextView
                  icon="person"
                  :text="
                    props.row.supplier.code + ' - ' + props.row.supplier.name
                  "
                />
                <div
                  :class="
            ...
            <q-td key="action" :props="props">
              <div class="flex justify-end no-wrap">
                ...
                <q-btn
                  v-if="$can('admin.supplier-wallet-transaction.delete')"
                  icon="more_vert"
                  rounded
                  dense
                  flat
