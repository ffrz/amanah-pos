...
const page = usePage();
const title = "Penyesuaian Saldo Deposit Suppllier";
...
// Fetch wallet_balance saat supplier_id berubah
watch(
  () => form.supplier_id,
  async (newCustomerId) => {
    if (!newCustomerId) {
    ...
    try {
      const res = await axios.get(
        route("admin.supplier.wallet_balance", { id: newCustomerId })
      );
...
const submit = () =>
  handleSubmit({
    form,
    url: route("admin.supplier-wallet-transaction.adjustment"),
  });
</script>
            ...
            <q-card-section class="q-pt-none">
              ...
              >
                <template v-slot:no-option>
                  <q-item>
                    <q-item-section>Suppllier tidak ditemukan</q-item-section>
                  </q-item>
                </template>
              </q-select>
              <LocaleNumberInput
