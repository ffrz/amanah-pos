...
const title = "Kartu Piutang Pelanggan";
const $q = useQuasar();
...
const deleteItem = (row) =>
  handleDelete({
    message: `Hapus ledger #${row.code}? Saldo pelanggan akan dikembalikan.`,
    url: route("admin.customer-ledger.delete", row.id),
...
const fetchItems = (props = null) => {
  ...
  handleFetchItems({
    ...
    props,
    rows,
    url: route("admin.customer-ledger.data"),
    loading,
    tableRef,
  });
};
...
<template>
  ...
  <authenticated-layout>
    ...
    <template #right-button>
      ...
      <q-btn
        v-if="false && $can('admin.customer-ledger.adjust')"
        icon="tune"
        dense
        rounded
        class="q-ml-sm"
        color="secondary"
        text-color="white"
        @click="router.get(route('admin.customer-ledger.adjust'))"
      >
      ...
      <q-btn
        v-if="$can('admin.customer-ledger.add')"
        icon="add"
        dense
        rounded
        class="q-ml-sm"
        color="primary"
        @click="router.get(route('admin.customer-ledger.add'))"
      >
      ...
      >
        ...
        <template v-slot:body="props">
          <q-tr
            ...
            @click.stop="
              router.get(
                route('admin.customer-ledger.detail', {
                  id: props.row.id,
                })
              )
            "
          >
            ...
            <q-td key="action" :props="props">
              <div class="flex justify-end no-wrap">
                ...
                <q-btn
                  v-if="
                    !props.row.ref_type && $can('admin.customer-ledger.delete')
                  "
                  icon="delete"
                  color="negative"
                  dense
