...
function ensureDeployListExists($ftp, string $remoteRoot): void
{
    ...
    foreach ([$localListPhp, $localListJson] as $localFile) {
        if (!file_exists($localFile)) {
            logMessage("❌ Missing local file: $localFile");
            exit;
        }
    }
    ...
    logMessage("📤 Uploading fresh copy of " . DEPLOY_LIST_SCRIPT_FILENAME . "...");
    if (ftp_put($ftp, $remoteListPhp, $localListPhp, FTP_BINARY)) {
        logMessage("✅ Uploaded " . DEPLOY_LIST_SCRIPT_FILENAME . " to /public");
    } else {
        logMessage("⚠️ Failed to upload " . DEPLOY_LIST_SCRIPT_FILENAME);
        exit;
    }

    // force upload config
    $remoteListJson = "$publicDir/" . DEPLOY_LIST_CONFIG_FILENAME;
    logMessage("📤 Uploading fresh copy of " . DEPLOY_LIST_CONFIG_FILENAME . "...");
    if (ftp_put($ftp, $remoteListJson, $localListJson, FTP_BINARY)) {
        logMessage("✅ Uploaded " . DEPLOY_LIST_CONFIG_FILENAME);
    } else {
        logMessage("⚠️ Failed to upload " . DEPLOY_LIST_CONFIG_FILENAME);
        exit;
    }
}
...
function fetchRemoteFileList(string $domain, string $token): array
{
    ...
    if (!is_array($data)) {
        logMessage("⚠️ Invalid JSON response from " . DEPLOY_LIST_SCRIPT_FILENAME);
        return [];
    }
        ...
        if (is_dir($includeReal)) {
            ...
            foreach ($iterator as $file) {
                ...
                if (!ftp_put($ftp, $remotePath, $filePath, FTP_BINARY)) {
                    logMessage("❌ Upload failed: $realPath");
                } else {
                    logMessage("✅ Uploaded: $realPath");
                }
            }
        } else {
            // Single file include
...
function patch(string $domain, string $token): void
{
    ...
    logMessage("🧩 Patch: " . ($data->message ?: "No response"));
}
...
function main(array $argv): void
{
    ...
    if (!file_exists($configFile)) {
        logMessage("❌ Missing " . CONFIG_FILENAME . " at $configFile");
        exit(1);
    }

    $cfg = json_decode(file_get_contents($configFile), true);
    if (!$cfg) {
        logMessage("❌ Invalid " . CONFIG_FILENAME);
        exit(1);
    }
